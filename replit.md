...
**Bilingual status (Arabic/English):** COMPLETE. Both dashboards fully bilingual via `useCLang()` hook and `CLangProvider`. Globe icon toggle in every screen. `dir={dir}` used everywhere — zero hardcoded `dir="rtl"` in both CompanyDashboard.tsx and ASABPrototype.tsx. All page wrappers, modals, tables, and inner components switch layout direction and text on toggle. Pages covered in CompanyDashboard.tsx: all CA pages, all Head pages, all Accountant pages (AccDashboard, AccCompanySales, AccCompanyExpenses, AccCompanyPurchases, AccCompanyInventory, AccInventoryItemsPage, AccCompanyAssets, AccCompanyShifts, AccCompanyReminders, AccCompanyWaste, AccCompanyEmployees, AccCompanyCash, AccCompanyReports), all Branch Manager pages, all Procurement pages (ProcOverview, ProcNew, ProcSuppliers, ProcItems, ProcReports, ProcGrouped, ProcSent), HeadBrands, ConvertToAssetModalCD, and all inner elements.
